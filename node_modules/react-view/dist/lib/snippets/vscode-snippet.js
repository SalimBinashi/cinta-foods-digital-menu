...
const getImportBody = (imports, props) => {
    const importList = imports ? utils_1.clone(imports) : {};
    // prop level imports (typically enums related) that are displayed
    // only when the prop is being used
    props &&
        Object.values(props).forEach((prop) => {
            if (prop.imports) {
                code_generator_1.addToImportList(importList, prop.imports);
            }
        });
    const importBody = [];
    let ctr = 1;
    ...
    if (props) {
        for (const propName in props) {
            ...
            if (propName === 'children')
                continue;
            if (props[propName].type === __1.PropTypes.Boolean) {
                const row = `  \${${ctr++}:${propName}}`;
                componentBody.push(row);
            }
            else if (props[propName].type === __1.PropTypes.Enum) {
            ...
            else if (props[propName].type === __1.PropTypes.String &&
                typeof props[propName].value === __1.PropTypes.String) {
